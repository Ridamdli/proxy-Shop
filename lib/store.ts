...
interface CartState {
  ...
  addToCart: (productId: string, quantity?: number, variantId?: string) => Promise<any>;
...
const loadCartFromStorage = (): { items: CartItem[], count: number, total: number } => {
...
const saveCartToStorage = (cart: { items: CartItem[], count: number, total: number }) => {
...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  clearCart: async () => {
    ...
    try {
      // Clear all items
      await Promise.all(get().items.map((item) => get().removeFromCart(item.id)));
    } catch (error) {
